...
public static final int ADMIN_ROLE = 4; // Here to check if current user is Employee/admin or not. Put as variable for easy access and repeated use 
	...
	public AbstractUser updateUser(HttpSession session, AbstractUser u) {
		if(session == null || session.getAttribute("currentUser") == null) {
			throw new NotLoggedInException();
		}
		
		AbstractUser currentUser = (AbstractUser) session.getAttribute("currentUser"); // Stored as 'AbstractUser' so casting should be ok.
		if((currentUser.getRole().getRoleId() < ADMIN_ROLE) || (u.equals(currentUser))) { // If not admin or the user
			throw new NotLoggedInException();
		}
